...
export function AuthPage({ onAuthenticated }: AuthPageProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      await login(mode === 'signup' ? name.trim() : undefined);
      onAuthenticated();
    } catch (loginError) {
      setError(
        ...
        <section className="mx-auto w-full max-w-md">
          ...
          <Card className="border-gray-200 shadow-xl shadow-gray-200/60">
            <CardBody className="p-8 sm:p-10">
              ...
              {!isEnokiConfigured && (
                <p className="mb-4 rounded-lg bg-amber-50 p-3 text-sm font-medium text-amber-800" role="alert">
                  Google sign-in needs the VITE settings in .env.local. See README-INTEGRATION.md.
                </p>
              )}
              ...
              <Button
                ...
                disabled={!isEnokiConfigured || isSigningIn}
              >
                {isSigningIn
                  ? 'Waiting for Google…'
                  : mode === 'login'
              ...
              <p className="mt-4 text-center text-xs leading-relaxed text-gray-500">
                Google zkLogin creates your Sui Testnet account without a seed phrase. ShieldPass never receives your Google password.
              </p>
